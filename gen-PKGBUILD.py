...
pkgver_repo_base = versions.pkgver_repo_base
...
source_repo_url = "http://repo.radeon.com/amdgpu/{0}/ubuntu/".format(pkgver_repo_base)
...
class Package:
    ...
    def fill_arch_info(self, deb_info):
        ...
        sources.append(source_repo_url.replace(pkgver_base,"${major}").replace(pkgver_repo_base, "${repo_base}") + deb_info["Filename"].replace(pkgver_base,"${major}").replace(pkgver_build,"${minor}").replace(ubuntu_ver, "${ubuntu_ver}"))
    ...
    def toPKGBUILD(self):
        ...
        for info in self.deb_source_infos:
            ...
            ret += tmp_str.replace(str(pkgver_base), "${major}").replace(str(pkgver_build), "${minor}").replace(ubuntu_ver,"${ubuntu_ver}")
...
if not debugging:
    print(header_tpl.format(
        ...
        pkgver_repo_base=pkgver_repo_base,
