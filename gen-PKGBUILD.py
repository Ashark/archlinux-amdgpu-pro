...
source_repo_url = f"https://repo.radeon.com/amdgpu/{pkgver_repo_base}/ubuntu/"
...
class Package:
    ...
    def fill_arch_info(self, deb_info):
        ...
        sources.append(source_repo_url.replace(pkgver_base, "${major}").replace(pkgver_repo_base, "${repo_ver}") + deb_info["Filename"].replace(pkgver_base, "${major}")\
                       .replace("_"+pkgver_base_short, "_${major_short}").replace(pkgver_build, "${minor}").replace(f"{ubuntu_ver}_", "${ubuntu_ver}_"))
