...
package_hsakmt-roct-amdgpu () {
    pkgdesc="development environment for hsakmt-roct"
    license=('MIT')
    depends=("amdgpu-core-meta=${major}_${minor}-${pkgrel}" "glibc" "numactl")

    extract_deb "${srcdir}"/amdgpu-pro-${major}-${minor}-ubuntu-18.04/hsakmt-roct-amdgpu_1.0.9-${minor}_amd64.deb
    move_copyright
}

package_hsakmt-roct-amdgpu-dev () {
    pkgdesc="development environment for hsakmt-roct"
    license=('MIT')
    depends=("hsakmt-roct-amdgpu=${major}_${minor}-${pkgrel}")
